...
- **Annotation retention:** delete cascade says annotations are marked
  ...
  - **Resolved for Phase 07:** annotations table includes `documant_id` (FK to
...
- `POST /admin/alerts/{documant_id}/trigger` lets admins match one indexed document
  on demand.
...
- `GET /documents/{documant_id}/related` returns permission-filtered related
  documents for a source document.
- Related documents use existing Qdrant chunk vectors and the authenticated
  user's group IDs, exclude the source document, deduplicate by `documant_id`, and
