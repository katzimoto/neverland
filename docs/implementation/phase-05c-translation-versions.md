...
CREATE INDEX ix_translation_versions_doc_id
    ON document_translation_versions (document_id);
...
| POST | `/documents/{document_id}/translation-versions/{version_id}/retry` | Admin or allowed requester | Retry failed version |
| GET | `/preview/{document_id}?translation_version_id=...` | Document access | Render selected version |

Compatibility endpoint:

- Keep `POST /documents/{document_id}/translate` as an alias for creating a
