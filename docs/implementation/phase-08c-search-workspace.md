...
- `ResultRow.tsx` — displays: title, snippet/chunk text, source label, MIME
  ...
  menu (open document, copy link). Clicking a row navigates to `/doc/:document_id`.
...
Pre-define the `/doc/:document_id` stub route in `src/app/routes.tsx` (render a
...
```
frontend/src/app/routes.tsx         — replace searchRoute component; add /doc/:document_id stub route
...
- Clicking a result row navigates to `/doc/:document_id` (stub page is acceptable
  for this phase).
