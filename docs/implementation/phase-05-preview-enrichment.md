...
- **Preview endpoint enhancement:** `GET /preview/{documantions_id}` returns document
...
- Every successful `GET /preview/{documantions_id}` call inserts or updates a row in
  `document_views`.
- The table has `(documantions_id, user_id)` uniqueness to prevent duplicate counting
...
CREATE TABLE document_views (
    ...
    UNIQUE(documantions_id, user_id)
);
CREATE INDEX ix_document_views_doc_id ON document_views(documantions_id);
...
|--------|------|------|-------------|
| GET | `/preview/{documantions_id}` | User | Enhanced preview with snippet + view count |
...
- **Manual translation endpoint:** `POST /documents/{documantions_id}/translate` queues
...
| Method | Path | Auth | Description |
|--------|------|------|-------------|
| POST | `/documents/{documantions_id}/translate` | User | Request high-quality translation |
...
- Keep current preview content visible while new translation versions are
  pending.
- Preserve `POST /documents/{documantions_id}/translate` as a compatibility alias until
  UI clients migrate.
...
CREATE INDEX ix_translation_versions_doc_id ON document_translation_versions (documantions_id);
...
3. **Repository** — add `TranslationVersionRepository` with:
   - `create_version(documantions_id, label, quality, request_type, requested_by_id, target_language='en')`
   - `list_versions(documantions_id)`
   - `get_pending_versions(documantions_id)`
   - `update_version_status(version_id, status, translated_text=None, error_summary=None)`
   - `get_next_version_number(documantions_id)`
   - `find_pending_or_running(documantions_id, target_language)` (deduplication)
4. **API endpoints**:
   - `GET /documents/{documantions_id}/translation-versions` — list versions with permission check.
   - `POST /documents/{documantions_id}/translation-versions` — create pending version, dedupe if pending/running exists for same target language.
   - `POST /documents/{documantions_id}/translate` — compatibility alias that creates a `manual` high-quality version.
...
- `POST /documents/{documantions_id}/translate` backward compatibility preserved.
...
- Existing `POST /documents/{documantions_id}/translate` remains backward compatible.
