...
1. **#184 / PR #197** — Dimension-based Qdrant collection isolation and vector safety.
   *Adds:* dimension property on encoder, Qdrant collection naming by dimension, dimension validation
   before upsert/search, `EMBEDDING_DIM` setting.
...
3. **#186** — Embedding model packaging and reindex procedure (operator-facing docs).
   *Adds:* air-gapped packaging/loading, OLLAMA_MODEL vs EMBEDDING_MODEL docs, reindex guide.
4. **#237 (this issue)** — Integration plan and docs (landed with #186 or as final doc PR).
...
**Yes.** Without #186, operators have no documented path to deploy a real embedding model or
reindex after a provider/dimension change. The feature branch is not complete until the
operator-facing docs exist.

## Qdrant collection naming/versioning strategy

- Collection name format: `documents_v{dimension}` (e.g. `documents_v384` for 384-dim vectors).
- When the embedding dimension changes, a new collection is created automatically by
  `QdrantSearchClient` on first use.
...
- Single source of truth: `EMBEDDING_DIM` environment variable (or `.env`), read by
  `shared.config.Settings.embedding_dim`.
- Encoder classes expose a class-level or instance-level `dimension` property that must match
  `EMBEDDING_DIM`.
- `build_encoder(settings)` in `services/search/factory.py` is the only production entry point.
  Test code must never call `build_encoder` with production settings.

## Model-change/reindex behavior

1. Update `EMBEDDING_MODEL` (and `EMBEDDING_DIM` if the new model uses a different dimension).
...
- [ ] #184 and #185 PRs are merged into `feature/vector-safety`.
...
- [ ] #186 docs are complete and reviewed.
- [ ] `CHANGELOG.md` updated for the vector safety track.

## Docs checks for #186

- [ ] `OLLAMA_MODEL` vs `EMBEDDING_MODEL` distinction explained.
- [ ] Air-gapped packaging and loading for embedding models documented.
- [ ] Verifying embedding model availability (health check or CLI command).
- [ ] Reindex procedure after model/provider/dimension change.
- [ ] Warning that deterministic-test vectors must not be used in production.
- [ ] Explanation of Qdrant collection isolation (how `documents_v{dim}` works).
