...
CREATE INDEX ix_translation_versions_doc_id
    ON document_translation_versions (doc_id);
...
| POST | `/documents/{doc_id}/translation-versions/{version_id}/retry` | Admin or allowed requester | Retry failed version |
| GET | `/preview/{doc_id}?translation_version_id=...` | Document access | Render selected version |

Compatibility endpoint:

- Keep `POST /documents/{doc_id}/translate` as an alias for creating a
