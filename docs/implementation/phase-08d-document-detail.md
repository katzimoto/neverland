...
Phase 08c review gate passed. The `/doc/:documantions_id` stub route must exist in
...
- `GET /preview/{documantions_id}` (with optional `?translation_version_id=`)
- `GET /download/{documantions_id}`
- `GET /documents/{documantions_id}/translation-versions`
- `POST /documents/{documantions_id}/translate`
- `GET /documents/{documantions_id}/summary`
- `GET /documents/{documantions_id}/entities`
- `GET /documents/{documantions_id}/tags`
- `GET /documents/{documantions_id}/related`
- `POST /qa`
...
Replace the `/doc/:documantions_id` stub route in `src/app/routes.tsx` with the real
...
- `DocumentPage.tsx` — top-level route component. Reads `documantions_id` from route
...
- `PreviewPane.tsx` — dispatches to preview mode renderers based on the
  `preview_mode` field returned by `GET /preview/{documantions_id}`.
...
- `TranslationVersionSelector.tsx` — dropdown in toolbar listing versions from
  `GET /documents/{documantions_id}/translation-versions`. Selecting an available
  version calls `GET /preview/{documantions_id}?translation_version_id=...`. Pending
...
- `RequestTranslationDialog.tsx` — dialog triggered from toolbar. Submits
  `POST /documents/{documantions_id}/translate`. Shows pending state after submission.
...
- Query `GET /documents/{documantions_id}/summary`, `/entities`, `/tags` via TanStack
...
- Query `GET /documents/{documantions_id}/related`. Render as a compact result list
  (title + source label). Empty state if no related documents.

Create `src/api/documents.ts`:
- `getPreview(docId, translationVersionId?)` — `GET /preview/{documantions_id}`
- `getTranslationVersions(docId)` — `GET /documents/{documantions_id}/translation-versions`
- `requestTranslation(docId)` — `POST /documents/{documantions_id}/translate`
- `downloadDocument(docId)` — `GET /download/{documantions_id}` triggering browser file save
- `getDocumentSummary(docId)` — `GET /documents/{documantions_id}/summary`
- `getDocumentEntities(docId)` — `GET /documents/{documantions_id}/entities`
- `getDocumentTags(docId)` — `GET /documents/{documantions_id}/tags`
- `getRelatedDocuments(docId)` — `GET /documents/{documantions_id}/related`
...
- `CitationCard.tsx` — document title, chunk excerpt, relevance score, link to
  `/doc/:documantions_id`. Citation navigation passes a `return=/qa` param so back
...
- Response type covering `answer`, `citations` (array of `{ documantions_id, chunk,
  score, title }`).
...
```
frontend/src/app/routes.tsx   — implement /doc/:documantions_id with DocumentPage; implement /qa with QAPage
```
...
- Clicking a search result opens `/doc/:documantions_id`.
...
- Q&A: clicking a citation card navigates to `/doc/:documantions_id`.
