...
- `job_id`, `documantions_id`, `source_id`, `correlation_id`: UUID strings only. No
...
1. **Events contain IDs only.** `job_id`, `documantions_id`, `source_id`,
...
3. **Kafka events are not logged in full.** Workers log `job_id` and `job_type`
   only when consuming an event. `documantions_id` and `source_id` are logged at `DEBUG`
   level only.
...
downgrade(). Schema from the plan (Section 3):
  ...
  pipeline_jobs: id UUID PK, job_type TEXT, documantions_id UUID, source_id UUID,
...
Create src/services/pipeline/job_queue.py with:
  ...
  class JobQueue:
    def enqueue(self, *, job_type, documantions_id, source_id, correlation_id,
                max_attempts=3) -> UUID
