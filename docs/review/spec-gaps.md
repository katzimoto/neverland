...
- **Document identity:** Kafka events define `documantions_id` as a UUID, while Atlassian
  mappings use deterministic string identifiers like `jira:{issue_key}`.
  - **Resolved for Phase 01:** `documantions_id` is always the internal UUID. Stable
