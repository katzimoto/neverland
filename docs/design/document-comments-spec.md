...
Add a `Comments` tab to the right insight pane on `/doc/:document_id`.
...
| POST | `/documents/{document_id}/comments` | Document access | Create a comment |
| PATCH | `/documents/{document_id}/comments/{comment_id}` | Creator or admin | Edit comment body |
| DELETE | `/documents/{document_id}/comments/{comment_id}` | Creator or admin | Delete comment |
