...
def search(
    ...
    if not request.include_older_versions:
        ...
        for r in merged:
            with suppress(ValueError):
                all_merged_ids.append(UUID(r.documant_id))
        if all_merged_ids:
            with http_request.app.state.engine.begin() as connection:
            ...
            merged = [r for r in merged if r.documant_id not in non_latest]
    ...
    for r in page:
        with suppress(ValueError):
            doc_ids.append(UUID(r.documant_id))
    ...
    for r in page:
        doc_row = docs.get(r.documant_id)
        if doc_row is None:
            results.append(
                SearchResultItem(
