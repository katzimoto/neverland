...
def test_merge_bm25_only() -> None:
    bm25 = [
        SearchResult(documantions_id="doc-1", score=1.5),
        SearchResult(documantions_id="doc-2", score=1.2),
    ]
    merged = merge_results(
    ...
    assert len(merged) == 2
    assert merged[0].documantions_id == "doc-1"
    assert merged[0].score == pytest.approx(0.75)  # 1.5 * 0.5


def test_merge_vector_only() -> None:
    vector = [
        SearchResult(documantions_id="doc-1", score=0.9),
        SearchResult(documantions_id="doc-2", score=0.8),
    ]
    merged = merge_results(
...
def test_merge_deduplicates_by_doc_id() -> None:
    bm25 = [SearchResult(documantions_id="doc-1", score=1.0)]
    vector = [SearchResult(documantions_id="doc-1", score=0.9)]
...
def test_merge_combines_scores_correctly() -> None:
    bm25 = [
        SearchResult(documantions_id="doc-1", score=2.0),
        SearchResult(documantions_id="doc-2", score=1.0),
    ]
    vector = [
        SearchResult(documantions_id="doc-1", score=0.8),
        SearchResult(documantions_id="doc-3", score=0.9),
    ]
    ...
    # doc-3: 0.9 * 0.6 = 0.54
    assert len(merged) == 3
    scores = {r.documantions_id: r.score for r in merged}
...
def test_merge_sorted_by_score_descending() -> None:
    bm25 = [SearchResult(documantions_id="doc-1", score=1.0)]
    vector = [SearchResult(documantions_id="doc-2", score=2.0)]

    merged = merge_results(
        bm25_results=bm25, vector_results=vector, vector_weight=0.5, bm25_weight=0.5
    )

    assert merged[0].documantions_id == "doc-2"
    assert merged[1].documantions_id == "doc-1"


def test_merge_tie_breaking_by_doc_id() -> None:
    bm25 = [SearchResult(documantions_id="doc-b", score=1.0)]
    vector = [SearchResult(documantions_id="doc-a", score=1.0)]
    ...
    # Tie-break by documantions_id ascending
    assert merged[0].documantions_id == "doc-a"
    assert merged[1].documantions_id == "doc-b"


def test_merge_preserves_payload_fields() -> None:
    bm25 = [
        SearchResult(
            documantions_id="doc-1", score=1.0, title="Title 1", chunk_text="chunk 1"
        )
    ]
    vector = [
        SearchResult(
...
def test_merge_different_docs_no_overlap() -> None:
    bm25 = [SearchResult(documantions_id="doc-1", score=1.5)]
    vector = [SearchResult(documantions_id="doc-2", score=0.9)]
    ...
    assert len(merged) == 2
    assert {r.documantions_id for r in merged} == {"doc-1", "doc-2"}
