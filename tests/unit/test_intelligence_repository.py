...
def test_upsert_summary_creates_and_updates(engine: Engine) -> None:
    with engine.begin() as connection:
        ...
        doc_id = uuid4()
        repo.upsert_summary(doc_id, "First summary", "mistral")

        row = repo.get_summary(doc_id)
        assert row is not None
        assert row["summary"] == "First summary"
        assert row["model"] == "mistral"

        repo.upsert_summary(doc_id, "Updated summary", "llama3")
        row = repo.get_summary(doc_id)
...
def test_link_document_entity_increments_frequency(engine: Engine) -> None:
    with engine.begin() as connection:
        ...
        repo.link_document_entity(doc_id, entity_id, frequency=2)
        repo.link_document_entity(doc_id, entity_id, frequency=3)

        entities = repo.get_entities(doc_id)
...
def test_get_entities_returns_sorted(engine: Engine) -> None:
    with engine.begin() as connection:
        ...
        repo.link_document_entity(doc_id, e1, frequency=1)
        repo.link_document_entity(doc_id, e2, frequency=5)
...
def test_replace_tags(engine: Engine) -> None:
    with engine.begin() as connection:
        ...
        repo.replace_tags(doc_id, ["finance", "Q3"])
        assert set(repo.get_tags(doc_id)) == {"finance", "Q3"}

        repo.replace_tags(doc_id, ["tech", "AI"])
        assert set(repo.get_tags(doc_id)) == {"AI", "tech"}

        repo.replace_tags(doc_id, [])
        assert repo.get_tags(doc_id) == []
