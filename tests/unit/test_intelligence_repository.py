    ...
    with eng.begin() as conn:
        conn.execute(sa.text("""
            CREATE TABLE document_summaries (
                documantions_id TEXT PRIMARY KEY,
...
def test_upsert_summary_creates_and_updates(engine: Engine) -> None:
    with engine.begin() as connection:
        ...
        repo.upsert_summary(documantions_id, "First summary", "mistral")

        row = repo.get_summary(documantions_id)
        assert row is not None
        assert row["summary"] == "First summary"
        assert row["model"] == "mistral"

        repo.upsert_summary(documantions_id, "Updated summary", "llama3")
        row = repo.get_summary(documantions_id)
...
def test_link_document_entity_increments_frequency(engine: Engine) -> None:
    with engine.begin() as connection:
        ...
        repo.link_document_entity(documantions_id, entity_id, frequency=2)
        repo.link_document_entity(documantions_id, entity_id, frequency=3)

        entities = repo.get_entities(documantions_id)
...
def test_get_entities_returns_sorted(engine: Engine) -> None:
    with engine.begin() as connection:
        ...
        repo.link_document_entity(documantions_id, e1, frequency=1)
        repo.link_document_entity(documantions_id, e2, frequency=5)
...
def test_replace_tags(engine: Engine) -> None:
    with engine.begin() as connection:
        ...
        repo.replace_tags(documantions_id, ["finance", "Q3"])
        assert set(repo.get_tags(documantions_id)) == {"finance", "Q3"}

        repo.replace_tags(documantions_id, ["tech", "AI"])
        assert set(repo.get_tags(documantions_id)) == {"AI", "tech"}

        repo.replace_tags(documantions_id, [])
        assert repo.get_tags(documantions_id) == []
