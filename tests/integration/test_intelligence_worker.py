...
def test_worker_summarizes_and_stores(
    ...
    with migrated_engine.begin() as connection:
        ...
        worker.process_document(documantions_id, content)

    with migrated_engine.begin() as connection:
        repo = IntelligenceRepository(connection)
        summary = repo.get_summary(documantions_id)
    ...
    mock_es.update_document_field.assert_any_call(
        str(documantions_id), "summary", "A document about AI and finance."
    )
...
def test_worker_auto_tags(
    ...
    with migrated_engine.begin() as connection:
        repo = IntelligenceRepository(connection)
        tags = repo.get_tags(documantions_id)
