...
def test_list_translation_versions(
    ...
    with migrated_engine.begin() as connection:
        ...
        version_repo.create_version(
            documantions_id=UUID(documantions_id),
            label="Manual en",
            quality="high",
            request_type="manual",
            target_language="en",
    ...
    response = client.get(
        f"/documents/{documantions_id}/translation-versions",
...
def test_preview_with_translation_version(
    ...
    response = client.get(
        f"/preview/{documantions_id}?translation_version_id={version_id}",
        headers={"Authorization": f"Bearer {token}"},
    )
...
def test_slow_worker_with_version_repository(
    ...
    with migrated_engine.begin() as connection:
        ...
        version_id = version["id"]
        doc_repo = DocumentRepository(connection)
        doc_repo.update_translation_quality(UUID(documantions_id), "pending_high")
