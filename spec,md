...
* `POST /search`
  ...
  * Returns: `{ results: [{ doc_id, title, summary, score, tags, translation_quality, source }], total }`

**Q&A (feature-flagged: `feature.rag_qa`)**
* `POST /qa`
  * Body: `{ question, group_filter?: [group_ids] }`
  * Returns: `{ answer: "...", citations: [{ doc_id, title, chunk, score }], model: "mistral", latency_ms: 420 }`
* Fails gracefully with 503 if Ollama is unreachable
* Logs to `user_activity` with `action='qa'`

**Preview**
* `GET /preview/{doc_id}`
  * Returns: `{ doc_id, preview_mode, content, content_type, summary?, annotations?, entities? }`
...
* **Delete:** `DELETE /doc/{doc_id}` # admin only
...
-> `index_qdrant()` # vector per chunk, payload: `{ doc_id, group_id, chunk_index }`
...
def publish_intelligence(doc_id, content_english, group_id):
    ...
    if tasks:
        publish("documents.intelligence", { doc_id, content_english, group_id, tasks })
```
...
".join([
    ...
    return QAResponse(
        answer=answer,
        citations=[{ doc_id: c.doc_id, chunk: c.text, score: c.score } for c in chunks],
