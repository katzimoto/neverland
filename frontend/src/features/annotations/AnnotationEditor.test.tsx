...
vi.mock("@/api/annotations", () => ({ createAnnotation: mocks.createAnnotation, updateAnnotation: mocks.updateAnnotation }));
...
test("sends create annotations through the backend-compatible API boundary", async () => {
  ...
  await waitFor(() => expect(mocks.createAnnotation).toHaveBeenCalledWith("d1", { body: "shared note", shared: true, position: null }));
});

test("sends update annotations with body shared and position for API mapping", async () => {
  const user = userEvent.setup();
  render(<AnnotationEditor docId="d1" annotation={{ id: "a1", doc_id: "d1", author_id: "u1", author_name: "Ari", body: "old note", position: { page: 4 }, shared: false, created_at: "2026-05-01T10:00:00Z" }} />);
  ...
  await waitFor(() => expect(mocks.updateAnnotation).toHaveBeenCalledWith("a1", { body: "updated note", shared: true, position: { page: 4 } }));
});
