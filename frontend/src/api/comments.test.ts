...
test("maps the backend comments envelope and query params", async () => {
  ...
  await expect(listComments("d1", { limit: 10, offset: 5, sort: "-created_at" })).resolves.toEqual([
    { id: "c1", document_id: "d1", author_id: "u1", author_name: "Ari", author: undefined, body: "Hello", created_at: "2026-05-01T10:00:00Z", updated_at: "2026-05-01T11:00:00Z" },
  ]);
...
test("maps ascending frontend sort to backend oldest", async () => {
  get.mockResolvedValueOnce({ document_id: "d1", comments: [], total: 0, skip: 0, limit: 50 });
