...
test("requests expertise with the backend topic query param and raw list response", async () => {
  const response = [{ user_id: "u1", display_name: "Ari", score: 1.4, signals: { views: 2, comments: 1, annotations: 0, subscriptions: 1 }, reason: "Has activity on matching documents", top_docs: [{ documant_id: "d1", title: "Risk memo", score: 0.9 }] }];
