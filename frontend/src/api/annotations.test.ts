...
test("maps the backend annotations envelope and privacy fields", async () => {
  ...
  await expect(listAnnotations("d1")).resolves.toEqual([
    { id: "a1", documantions_id: "d1", author_id: "u1", author_name: "Ari", body: "Shared note", position: { page: 2 }, shared: true, created_at: "2026-05-01T10:00:00Z", updated_at: "2026-05-01T11:00:00Z" },
  ]);
});

test("maps create and update writes to backend annotation payloads", async () => {
  const response = { id: "a1", documantions_id: "d1", user_id: "u1", user_display_name: "Ari", text: "Private note", position: { page: 1 }, is_private: true, created_at: "2026-05-01T10:00:00Z", updated_at: "2026-05-01T10:00:00Z" };
